...
void mips_detect_memory()
{
    ...
    maxpa = 67108864;//65536K
    npage = 16384;//页表的数量 = 64M/4K = 16*2^10 = 16384
    basemem = 67108864;//65536K
    extmem = 0;
...
{   //分配n字节的物理内存。其实只是把freemem这一标志空闲内存的位置增加了一些。
    //最终返回的是这一页表的初始的虚拟地址
    extern char end[];
    u_long alloced_mem;
...
{//返回一个页表里的值的指针
     ...
     * entry value. */
    pgdir_entryp = &pgdir[PDX(va)];//先通过PDX(va)获得一个虚拟地址对应的页目录索引（偏移量），然后在（一级）页目录中找到这项吧
	//pgdir_entryp 是某个二级页表物理地址???
    /* Step 2: If the corresponding page table is not exist and parameter `create`
     * is set, create one. And set the correct permission bits for this new page
     * table. */
	//表头第一项有没有数值，且该数值有效位是不是1，如果不是。。。说明页表不存在
    if ((!(*pgdir_entryp & PTE_V)) && (create == 1)){
        *pgdir_entryp = PADDR( (Pde)alloc(BY2PG,BY2PG,1) | PTE_V );//先分配4KB的虚拟内存并且有效位置1，然后转化成物理地址
        //printf("boot_pgdir_walk %x\n", *pgdir_entryp);
    }
    pgtable = (Pte *)KADDR(PTE_ADDR(*pgdir_entryp));//把这个物理地址转化为内核虚地址
    pgtable_entry = &pgtable[PTX(va)];//通过PTX(va)获得页表索引（偏移量），然后在（二级）页表中找到这项吧
    return 	pgtable_entry;//最后得到页面的物理地址？？
}
...
void boot_map_segment(Pde *pgdir, u_long va, u_long size, u_long pa, int perm)
{
    ...
    for (i = 0; i < size; i+=BY2PG){
        va_temp = va+i;
        pgtable_entry = boot_pgdir_walk(&pgdir, va_temp, 1);//Big Change from 0 to 1//求出va_temp对应的那个物理地址的下标
...
void mips_vm_init()
{
    ...
    Pde *pgdir;//页目录项 page directory entry
    u_int n;

    /* Step 1: Allocate a page for page directory(first level page table). */
	//分配4096Byte的物理内存的空间给页目录，此处返回的pgdir是一个内核虚地址？
...
page_init(void)
{
    ...
    // printf("HHHH %x \n", freemem);
    int i;
    for (i = 0; i < (freemem - ULIM) / BY2PG; ++i){
        //for (i = 0; i < (freemem - 0x80400000) / BY2PG; ++i){
        pages[i].pp_ref = 1;
        //LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
	}

    /* Step 4: Mark the other memory as free. */
    for (i = (freemem-ULIM)/BY2PG; i < npage; ++i){
        //for (i = (freemem - 0x80400000) / BY2PG; i < npage; ++i){
        pages[i].pp_ref = 0;
        LIST_INSERT_HEAD(&page_free_list, &pages[i], pp_link);
        //printf("%x ",&pages[i]);
    }
...
{//从空闲的内存中分配物理页,可以理解为从page_free_list中拿出来一个
    ...
    //bzero((void *)ppage_temp, BY2PG);
    bzero(page2kva(ppage_temp), BY2PG);
    *pp = ppage_temp;//最终pp的值是一个从page_free_list的头部取得的一个
    return 0;
}
...
/*Overview:
 ...
 If we're out of memory, return -E_NO_MEM.
	...
	ppte -> physical page table entry
...
{//pgdir是一个二级页表；返回页表的入口给ppte，
    ...
    pgdir_entryp = pgdir+PDX(va);//找到
    /* Step 2: If the corresponding page table is not exist(valid) and parameter `create`
     * is set, create one. And set the correct permission bits for this new page
     * table.
     * When creating new page table, maybe out of memory. */
    if (!(*pgdir_entryp & PTE_V) && (create == 1)){//找到的这个页表*pgdir_entry(物理地址) 不存在的话，就新建一个
        if (page_alloc(&ppage) != 0)//如果分配一个新的页面失败的话
            return -E_NO_MEM;
        ppage->pp_ref ++;
        *pgdir_entryp =  (Pde)page2pa(ppage) | PTE_V ;
        pgtable = page2kva(ppage);
        //		printf("pgdir_work%x  %x  %x %x   %x\n",pgdir_entryp, *pgdir_entryp, page2pa(ppage),PTE_ADDR(*pgdir_entryp),ppage);
    ...
    *ppte = pgtable + PTX(va);
	//*ppte = &pgtable[PTX(va)];
    //	printf("pgtable ppte *ppte **ppte %x   %x %x   %x\n",pgtable,ppte,*ppte,**ppte);
    return 0;
}
...
{//修改页表，添加虚拟地址到物理页面的映射，钦定一个struct Page *pp
    u_int PERM;
    Pte *pgtable_entry;
    PERM = perm | PTE_V;
	...
	//最终得到的物理地址放在pgtable_entry这个指针里

    if (pgtable_entry != 0 && (*pgtable_entry & PTE_V) != 0) {
        if (pa2page(*pgtable_entry) != pp) {
            page_remove(pgdir, va);//如果已经存在了一个va映射的页表，先删去这个映射关系
        } else	{
