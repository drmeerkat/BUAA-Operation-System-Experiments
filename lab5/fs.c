...
unmap_block(u_int blockno)
{
	...
	if (syscall_mem_unmap(0, diskaddr(blockno)) < 0) {
...
write_block(u_int blockno)
{
	...
	va = diskaddr(blockno);//得到文件系统进程中的磁盘块虚拟地址
...
free_block(u_int blockno)
{
	...
	if (blockno == 0) {
		user_panic("blockno cannot be 0");
	}
	// Step 2: Update the flag bit in bitmap.
	bitmap[blockno / 32] |= 1 << (blockno % 32);//指导书说置0？应该是1吧。。。。
}
// Overview:
...
dir_lookup(struct File *dir, char *name, struct File **file)
{
	...
	for (i = 0; i < nblock; i++) {
		...
		if (file_get_block(dir, i, &blk) < 0) {
			user_panic("file get block failed: %d", i);
		}
