...
void init_disk() {
	...
	//此处加上1023是为了防止除法截断，比如当NBLOCK不足1024个的时候，除法结果会变成0
	nextbno = 2 + nbitblock;
	...
	for (i = 0; i < nbitblock; ++i) {
		...
		//把这一个block全部填写1，因为数据是一个字节(byte = 8bit),所以需要NBlOCK/8
		//表示往disk[2+i].data一共写这么多个字节的数据？
	}
	if (NBLOCK != nbitblock * BY2BLK) {
		//如果不相等，说明block个数不是1024的倍数，标记的时候用的块不能全部填1，存在没用的块。要把那部分改回成0
		diff = NBLOCK % BY2BLK / 8;//最后没填满的那一块的开头那段有效的长度，字节为单位？？
...
struct File *create_file(struct File *dirf) {
	...
	// no spare file link, just make a new one.
